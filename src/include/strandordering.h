...
class orderingList {
public:
	orderingList(int insize, int in_id, char *inTag, char *inSeq, BaseType *inBaseSeq, char* inStruct);
	~orderingList(void);
...
public:
	StrandOrdering(void);
	StrandOrdering(char *in_seq, char *in_struc, BaseType *in_bseq);
	StrandOrdering(orderingList *beginning, orderingList *ending, int numitems);
	StrandOrdering(char *in_seq, char *in_structure, BaseType *in_bseq, class identList *strandids);
	~StrandOrdering(void);
	void cleanup(void);
	...
	void addBasepair(BaseType *first_bp, BaseType *second_bp);
	void breakBasepair(BaseType *first_bp, BaseType *second_bp);
	...
	void generateFlatSequence(char **sequence, char **structure, BaseType **code_sequence, bool debug);

	// this function converts an index into a previously given sequence from generateFlatSequence into a BaseType * pointer into the appropriate strand's sequence at the given location.
	BaseType *convertIndex(const int index);
	...
	// as the updated index into that open loop only, and a BaseType * pointing at
	// the particular base in the open loop.
	OpenLoop* getIndex(JoinCriteria&, int, BaseType **location, bool);
